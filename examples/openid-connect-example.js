...
app.get('/user/foo', oidc.check('foo'), function(req, res, next) {
  /*jshint unused:false */
  res.send('<h1>Success!</h1><p>You are now viewing a page restricted by the "foo" scope.</p>');
});

app.get('/user/bar', oidc.check('bar'), function(req, res, next) {
  /*jshint unused:false */
  res.send('<h1>Success!</h1><p>You are now viewing a page restricted by the "bar" scope.</p>');
});

app.get('/user/and', oidc.check('bar', 'foo'), function(req, res, next) {
  /*jshint unused:false */
  res.send('<h1>Success!</h1><p>You are now viewing a page restricted by the "bar and foo" scopes.</p>');
});

app.get('/user/or', oidc.check(/bar|foo/), function(req, res, next) {
  /*jshint unused:false */
  res.send('<h1>Success!</h1><p>You are now viewing a page restricted by the "bar or foo" scopes.</p>');
});
        ...
        if(options) {
          for(var j in options) {
            if(options.hasOwnProperty(j)) {
              inp +=  '<div>'+
                '<label for="'+name+'_'+j+'">'+options[j]+'</label>'+
                '<input id="'+name+'_'+j+' name="'+name+'" type="'+(type||'radio')+'" value="'+j+'"'+(value&&value == j?' checked':'')+'>'+
                '</div>';
            }
          }
        } else {
    ...
    case '3':
      ...
      var post_options = {
        ...
        headers: {
          ...
          'Authorization': 'Basic '+new Buffer(test.client_id, 'utf8').toString('base64'),
